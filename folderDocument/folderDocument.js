...
document.querySelector(".init").addEventListener("click", event => {
    ...
    if(flagKeyboard === true) {
        ...
        console.log(link)
        console.log(link.id)
        localStorage.setItem('documentFolder', JSON.stringify(data.folderDocument[link.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    } else if(flagKeyboard === false && !linkThree) {
        console.log(linkTwo)
        console.log(linkTwo.id)
        localStorage.setItem('documentFolder', JSON.stringify(data.folderDocument[linkTwo.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    } else {
        console.log(linkThree);
        console.log(linkThree.id);
        localStorage.setItem('documentFolder', JSON.stringify(data.folderDocument[linkThree.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    }
