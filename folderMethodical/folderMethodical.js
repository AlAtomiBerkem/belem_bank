...
document.querySelectorAll('.documentFon').forEach(item => {
    item.addEventListener('click', event => {
        localStorage.setItem('documentFolder', JSON.stringify(data.folderMethodical[event.target.id]))
...
document.querySelector(".init").addEventListener("click", event => {
    ...
    if(flagKeyboard === true) {
        console.log('хуй1111')
        localStorage.setItem('documentFolder', JSON.stringify(data.folderMethodical[link.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    } else if(flagKeyboard === false && !linkThree) {
        console.log('хуй22222')
        localStorage.setItem('documentFolder', JSON.stringify(data.folderMethodical[linkTwo.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    } else {
        localStorage.setItem('documentFolder', JSON.stringify(data.folderMethodical[linkThree.id]))
        localStorage.setItem('idDocumentFolder', JSON.stringify(link.id))
        document.body.style.opacity = 0;
    }
    setTimeout(() => {
...
document.querySelector('.simplebar-scrollbar').addEventListener('click', e => {
    console.log(1000)
})
